...
def train_d(net, data, label="real"):
    ...
    if label=="real":
        ...
        err.backward()
        return pred.mean(), rec_all, rec_small, rec_part
    else:
        pred = net(data, label)
        err = F.relu( torch.rand_like(pred) * 0.2 + 0.8 + pred).mean()
        err.backward()
...
def train(args):
    ...
    for iteration in range(current_iteration, total_iterations+1):
        ...
        if iteration % save_interval == 0:
            print("GAN: loss d: %.5f    loss g: %.5f"%(err_dr.item(), -err_g.item()))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--path', type=str, default='./list/places2_train.flist', help='path of resource dataset, should be a folder that has one or many sub image folders inside')
    parser.add_argument('--cuda', type=int, default=0, help='index of gpu to use')
    parser.add_argument('--name', type=str, default='test1', help='experiment name')
    parser.add_argument('--iter', type=int, default=5000000, help='number of iterations')
